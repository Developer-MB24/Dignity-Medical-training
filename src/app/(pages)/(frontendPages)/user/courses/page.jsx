...
const fetchEnrollmentUser = async (id) => {
  try {
...
const CoursesPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        ...
        const studentenrollment = await fetchEnrollmentUser(id);
        setEnrollmentData(enrollment);
        setStudentEnrollmentData(studentenrollment);
      }
  ...
  // console.log("student enroll", studentenrollmentData);
  ...
  if (!studentenrollmentData || studentenrollmentData.length < 0)
    return <p>Course not found or its empty</p>;
  return (
    <DashboardLayout>
      <div className="text-2xl font-bold">Your Enrolled Course...</div>
      <div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mt-4">
          {(loading ? placeholderData : studentenrollmentData).map(
            (card, index) => (
              <Link key={index} href={card.href || `/user/courses/${card.id}`}>
                <div className="relative flex flex-col bg-clip-border rounded-xl overflow-hidden bg-gradient-to-tr from-white/65 to-white/95 border-black/80 text-white shadow-gray-900/20 shadow-md h-full">
                  ...
                  <div className="absolute bottom-0 w-full p-4 bg-black bg-opacity-60 text-left">
                    ...
                    <p className="block font-sans text-sm antialiased font-normal leading-normal text-primarygold uppercase">
                      <span className="text-white">Course ID:</span>{" "}
                      {card.course_id}
                    </p>
                    <p className="block font-sans text-sm antialiased font-normal leading-normal text-primarygold uppercase">
            ...
            <div className="flex justify-center space-x-4">
              {coursesData
                ...
                .map((card, index) => (
                  ...
                  >
                    <div className="relative h-64">
                      <Link href={`/courseDetailPage/${card.id}`}>
                        <Image
                          src={card.img || "/images/pages/first_aid.jpg"}
                    ...
                    <div className="absolute bottom-0 w-full p-4 bg-black bg-opacity-60 flex flex-col justify-between">
                      <div className="flex justify-between mb-2">
                        <Link href={`/courseDetailPage/${card.id}`}>
                          <p className="block font-sans text-sm antialiased font-normal leading-normal text-primarygold hover:text-goldlight cursor-pointer uppercase">
                            {card.course_title}
                          </p>
                        </Link>
                        <p className="block font-sans text-sm antialiased font-normal leading-normal text-primarygold uppercase">
